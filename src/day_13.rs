...
fn calculate_prize(cheat_codes: &Vec<(f128, f128, f128, f128, f128, f128)>, precision: bool)->i128 {
    let mut total_tokens: i128 = 0; 
    
    for &(r, u, s, v, mut x, mut y) in cheat_codes {
        if precision {
            x = x + 10_000_000_000_000.0;
            y = y + 10_000_000_000_000.0;
        } 
        let (a, b)=calculate_press(r, s, u, v, x, y);
        let res_x = a as f128*r + b as f128*s;
        let res_y = a as f128*u + b as f128*v;
        if res_x==x && res_y==y {
            total_tokens+= (a*3)+b;
        }
    }
...
fn calculate_press(r: f128, s: f128, u: f128, v: f128, x: f128, y: f128)->(i128, i128) {
    let nom = r*y - u*x;
    let denom = r*v - u*s;
    let b = nom / denom;
    let a = (x-s*b)/r;
...
fn parse_input(source: &str)->Vec<(f128, f128, f128, f128, f128, f128)> {
    let find = r"Button\sA\:\sX\+(\d+)\,\sY\+(\d+)\nButton\sB\:\sX\+(\d+)\,\sY\+(\d+)\nPrize\:\sX\=(\d+)\,\sY\=(\d+)";
    let re = Regex::new(find).unwrap();

    let mut results = vec![];
    for (_, [r, s, u, v, x, y]) in re.captures_iter(source).map(|c| c.extract()) {
        results.push(
                    (r.parse::<f64>().unwrap_or(0.0) as f128, s.parse::<f64>().unwrap_or(0.0) as f128,
                        u.parse::<f64>().unwrap_or(0.0) as f128, v.parse::<f64>().unwrap_or(0.0) as f128, 
                        x.parse::<f64>().unwrap_or(0.0) as f128, y.parse::<f64>().unwrap_or(0.0) as f128 )
                    );
    }
...
fn main() {
    // we need to find a and b multipliers from:
    // X = R*a + S*b (1)
    // Y = U*a + V*b (2)
    ...
    // a = (Y-Vb)/U from (2)
    // a = a -> U[ (X-Sb) ] = R[(Y-Vb)]
    // UX - USb = RY - RVb -> b (RV - US) = RY - UX
    // b = (RY - UX) / (RV - US)
    
    // part 2 math, but not used!
    // 10_000_000_000_000 = 10e13 = Z
    // b = (R(Y+Z)- U(X+Z)) / (RV - US)
    // b = (RY + RZ - UX - UZ) / (RV-US)
    // b = [(RZ-UZ)+(RY-UX)] / (RV-US)
    // b = [Z(R-U)+(RY-UX)] / (RV-US)
    // b = Z(R-U)/(RV-US) + b from part 1
    // K = Z(R-U)/(RV-US)
    // a = (X-Sb)/R -> a = (X-S(K+b))/R
    // a = X/R - S/R(K+b)
    // a= X/R - SK/R - Sb/R -> a = (X-Sb)/R - SK/R 
    // a = a from part 1 - SK/R
    
    assert_eq!((80,40), calculate_press(94.0, 22.0, 34.0, 67.0, 8400.0, 5400.0));
...
Prize: X=18641, Y=10279";    
    ...
    assert_eq!(480, calculate_prize(&parse_input(&source), false));
    assert_eq!(875318608908, calculate_prize(&parse_input(&source), true));
...
Prize: X=9475, Y=4041";
    ...
    assert_eq!(29522, calculate_prize(&parse_input(&source), false));
    assert_eq!(29522, calculate_prize(&parse_input(&source), true));
