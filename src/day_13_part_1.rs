...
fn calculate_prize(cheat_codes: &Vec<(f64, f64, f64, f64, f64, f64)>)->i64 {
    let mut total_tokens: i64 = 0; 
    
    for &(r, u, s, v, x, y) in cheat_codes {
        let (a,b) = calculate_press(r, s, u, v, x, y);
        let res_x = a as f64*r + b as f64*s;
        let res_y = a as f64*u + b as f64*v;
...
fn calculate_press(r: f64, s: f64, u: f64, v: f64, x: f64, y: f64)->(i64, i64) {
...
fn parse_input(source: &str)->Vec<(f64, f64, f64, f64, f64, f64)> {
    ...
    for (_, [r, s, u, v, x, y]) in re.captures_iter(source).map(|c| c.extract()) {
        results.push(
                    (r.parse::<f64>().unwrap_or(0.0), s.parse::<f64>().unwrap_or(0.0),
                        u.parse::<f64>().unwrap_or(0.0), v.parse::<f64>().unwrap_or(0.0), 
                        x.parse::<f64>().unwrap_or(0.0), y.parse::<f64>().unwrap_or(0.0) )
                    );
    }
...
Prize: X=18641, Y=10279";    
    ...
    assert_eq!(480, calculate_prize(&parse_input(&source)));
...
Prize: X=9475, Y=4041";
    ...
    assert_eq!(29522, calculate_prize(&parse_input(&source)));
